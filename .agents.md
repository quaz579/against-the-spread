...
**Scope:**
- **Admin**: Upload weekly lines Excel file to Azure Blob Storage through the authenticated admin web UI
...
- **Testing**: xUnit, bUnit (Blazor), NSubstitute
...
- Admin routes (upload, current-admin) require Google Identity Services sign-in, validated
  server-side against `ADMIN_EMAILS`. See `AUTHENTICATION_FIX.md`.
- User-facing read/picks APIs remain anonymous by design.
...
**Admin Endpoints:**
- `POST /api/upload-lines`, `POST /api/upload-bowl-lines` - Upload Excel lines to blob storage
  (require Google Identity Services auth, see `AUTHENTICATION_FIX.md`)
- `GET /api/current-admin` - Identity check for the admin UI
...
### Implemented
- Google Identity Services authentication on admin endpoints, validated against `ADMIN_EMAILS`
...
1. **Pick History**: Show user's past picks and results
2. **Leaderboard**: Track weekly winners and total wins
3. **Automated Scoring**: Parse game results and calculate winners
4. **Push Notifications**: Remind users to submit picks
5. **Playoff Bracket**: Tournament-style picks
6. **Admin Dashboard**: View all submissions, generate reports
7. **Mobile Apps**: Native iOS/Android using .NET MAUI
